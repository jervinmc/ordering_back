...
class TransactionView(viewsets.ModelViewSet):  
    ...
    def create(self,request):
        ...
        serializers = NotificationSerializer(data={"user_id":1,"descriptions":f"You have a new order({res.get('product_name')}) from user_id {res.get('user_id')}","image":res.get('image'),"viewed":"no"})
        ...
        if(serializer.data[0]['stocks']<5):
            item1 = NotificationSerializer(data={"user_id":1,"descriptions":f"Running out of stocks product: {res.get('product_id')}","image":res.get('image')})
